...
export interface EquipmentState {
  ...
  cargadorEntregado: boolean;
  accesorios: string;
}

export interface EquipmentFormProps {
  equipo: EquipmentState;
  onChange: (equipo: EquipmentState) => void;
}

export function EquipmentForm({ equipo, onChange }: EquipmentFormProps) {
  const update = (field: keyof EquipmentState, value: string | boolean) => {
    onChange({
      ...equipo,
      [field]: value,
    });
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <InputField
        ...
        onChange={(event) => update("numeroSerie", event.target.value)}
      />

      <InputField
        label="Sistema operativo"
        value={equipo.sistemaOperativo || ""}
        onChange={(event) => update("sistemaOperativo", event.target.value)}
      />

      <InputField
        label="Procesador"
        value={equipo.procesador || ""}
        onChange={(event) => update("procesador", event.target.value)}
      />
      ...
      <InputField
        ...
        onChange={(event) => update("almacenamiento", event.target.value)}
      />

      <InputField
        label="Accesorios recibidos"
        value={equipo.accesorios || ""}
        onChange={(event) => update("accesorios", event.target.value)}
      />
