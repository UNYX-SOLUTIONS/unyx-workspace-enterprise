...
export default function ProformaHistoryPage() {
  ...
  const summary = useMemo(() => {
    const totalAmount = proformas.reduce((sum, proforma) => sum + toDisplayCents(proforma?.total), 0);
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard title="Total (página)" value={formatCurrency(summary.totalAmount)} icon="💵" color="blue" />
              ...
              <table className="w-full border-collapse text-left">
                <thead>
                  <tr className="border-b border-[#b7c3d7] bg-[#e7effd]">
                    ...
                    <th className="hidden px-4 py-4 text-right sm:table-cell sm:px-6">
                      <button type="button" onClick={() => handleSort("total")} className={`${sortableHeaderClass} justify-end`}>
                ...
                <tbody className="divide-y divide-[#d1d8e3]">
                  {sortedProformas.map((proforma) => {
                    ...
                    return (
                      <tr key={proforma.id || proforma.numero} className="bg-white transition-colors hover:bg-[#f3f7ff]">
                        ...
                        <td className="px-4 py-4 sm:px-6">
                          <p className="font-bold text-[#111827]">{proforma?.cliente?.nombre || "Sin cliente"}</p>
                        ...
                        <td className="hidden px-4 py-4 font-medium text-[#374151] sm:px-6 md:table-cell">
                          {proforma.fecha ? new Date(proforma.fecha).toLocaleDateString("es-EC") : "Sin fecha"}
                        </td>
                        <td className="px-4 py-4 sm:px-6">
                          <span
                        ...
                        <td className="px-4 py-4 sm:px-6">
                          <div className="flex justify-center gap-2">
                            <ActionButton
                              label={generatingPdfNumber === proforma.numero ? "Generando..." : "PDF"}
                              variant="secondary"
