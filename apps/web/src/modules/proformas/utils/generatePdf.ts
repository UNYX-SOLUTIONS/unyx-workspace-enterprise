...
export interface PdfProformaItem {
  codigo?: string | null;
...
export interface PdfProformaCliente {
  nombre?: string | null;
  ruc?: string | null;
...
export interface PdfProformaData {
  numero?: string | null;
  fecha?: unknown;
  validezDias?: number | string | null;
  notas?: string | null;
  cliente?: PdfProformaCliente | null;
  items?: PdfProformaItem[];
  subtotal?: unknown;
  iva?: unknown;
  total?: unknown;
}
...
function validateProforma(proforma: PdfProformaData): void {
...
export async function generatePdf(proforma: PdfProformaData): Promise<void> {
  ...
  autoTable(document, {
    startY: 105,
    head: [["ITEM", "DESCRIPCIÓN ITEM", "MARCA", "CANT.", "PRECIO", "SUBTOTAL"]],
  ...
  const lastAutoTableY =
    (document as unknown as { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? 0;

  let finalY = lastAutoTableY + 12;
